...
**Windows note:** use `go build -o gritt.exe .` so the binary gets the `.exe` extension — without it, cmd.exe/PowerShell/TCC won't find it by name. Then run it with a backslash, not the Unix `./` shown below (e.g. `.\gritt.exe -l`).
