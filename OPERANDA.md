...
- **CI + local reproduction (act + OrbStack)**: `.github/workflows/test.yml` has two jobs — `go` (fast, no Dyalog, runs unit tests; TUI/aplsock skip) and `dyalog` (full suite inside `dyalog/dyalog:latest`, with apt-installed tmux + zsh, runs as root for installation). `Makefile` coordinates `test-go` + `test-aplsock` locally without pkill. Locally reproducible via `act` routed through OrbStack — `~/Library/Application Support/act/actrc` pins `catthehacker/ubuntu:act-latest` and `--container-options=--init`. The `--init` is load-bearing: catthehacker's bash-PID-1 doesn't reap zombies, so `kill_wait_test.go`'s SIGKILL escalation tests fail because `processAlive(cmd)` (= `kill(-PG, 0)`) falsely reports orphan `sleep` zombies as alive. Production already handles this via `dyalogStillRunning` (Wait-aware) — comment on `tui.go:295`. Real GH ubuntu-latest runners have proper init, so this is purely an act-environment concern. Full how-to in CLAUDE.md "Running CI locally" section.
