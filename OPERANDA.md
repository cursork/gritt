...
- **CI + local reproduction (act + OrbStack)** — branch `tests-readme`. `.github/workflows/test.yml` has two jobs:
  - **`go`** — ubuntu-latest, no Dyalog. Vet + build + `go test -v ./...`. TestTUI and aplsock skip cleanly without Dyalog on PATH; everything else (codec, amicable, prepl unit, aplcart, aplor, ibeam, command/config) runs. Passes on real GH today.
  - **`dyalog`** — runs inside `dyalog/dyalog:latest` with `options: --user root --init`. Apt-installs `tmux zsh nodejs` (nodejs because the bare Dyalog image has no node and JS actions like `setup-go@v5` need one — real GH auto-mounts node into containers, act doesn't). Runs full Go suite + `grittles/aplsock/test.sh`. Mostly green — TestTUI's tracer tests still fail; see FACIENDA "CI: tracer pane doesn't open".

  Fixes that landed alongside the workflow:
  - `--init` on both job containers — catthehacker's and dyalog/dyalog's bash-PID-1 don't reap zombies, so `kill_wait_test.go`'s SIGKILL escalation tests false-failed because `processAlive(cmd)` (= `kill(-PG, 0)`) returns true on unreaped zombie children. Production code already handles this via `dyalogStillRunning` (Wait-aware) — comment on `tui.go:295`.
  - `main.go` `-log` arg now `mkdir -p`'s the file's parent. Was `log.Fatalf`'ing on a fresh CI checkout where `test-reports/` didn't exist (it's in `.gitignore`), which killed gritt before tmux could capture anything, manifesting as `tmux: exit status 1` because the session died with the process.
  - `tui_test.go` docs DB setup now uses an OS-correct path (`os.UserCacheDir()` with `HOME=/tmp` instead of the macOS-hardcoded `/tmp/Library/Caches/gritt`) and calls `docs.RefreshCache()` if no local copy exists, so doc-search tests work on fresh CI.
  - `uitest.StartDyalog` sets `RIDE_SPAWNED=1` to match `session.StartDyalog` (the runtime `-l` path). Doesn't on its own fix the tracer-in-container issue but keeps environments aligned.

  `Makefile` coordinates `test-go` + `test-aplsock` locally without `pkill`. Locally reproducible via `act` routed through **OrbStack** — `~/Library/Application Support/act/actrc` pins `catthehacker/ubuntu:act-latest` and `--container-options=--init`. OrbStack owns `/var/run/docker.sock` so act routes through its lightweight Linux VM rather than host Docker. Full how-to in CLAUDE.md "Running CI locally".
